...
def main():
    ...
    with tab_pred:
        ...
        # Visualisasi radar chart untuk risiko (DIPERBAIKI)
        ...
        # Perbaikan: Hitung sudut dan tutup poligon
        angles = np.linspace(0, 2 * np.pi, len(risk_labels), endpoint=False)
        angles = np.concatenate((angles, [angles[0]]))  # Menutup poligon
        risk_values_closed = np.concatenate((risk_values, [risk_values[0]]))  # Menutup poligon

        fig, ax = plt.subplots(figsize=(6, 6), subplot_kw=dict(polar=True))
        ax.fill(angles, risk_values_closed, 'red', alpha=0.25)
        ax.plot(angles, risk_values_closed, color='red', marker='o')
        ax.set_xticks(angles[:-1])  # Hanya label titik asli (bukan penutup)
        ax.set_xticklabels(risk_labels)
        ax.set_title("Profil Risiko Mahasiswa", size=14, pad=20)
        ax.set_ylim(0, 1.5)  # Batas radial (karena nilai maks 1.5)
