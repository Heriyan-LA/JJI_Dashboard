...
def plot_distributions(df, chart_type):
    ...
    if 'Status' in df.columns and grade_col:
...
def main():
    ...
    with tab3:
        ...
        if 'Status' in df_processed.columns:
            ...
            for status in df_processed['Status'].unique():
                if "dropout" in str(status).lower():
                    dropout_class = status
                    break
    ...
    with tab5:
        ...
        if pd.api.types.is_numeric_dtype(data[selected_feature]):
            ...
            st.warning(f"Fitur {selected_feature} bukan numerik, tidak dapat membuat visualisasi distribusi")
        ...
        col1, col2 = st.columns(2)
        with col1:
            x_feature = st.selectbox("Fitur X", numeric_features, index=0)
        with col2:
            y_feature = st.selectbox("Fitur Y", numeric_features, index=1 if len(numeric_features) > 1 else 0)
        
        # Pastikan fitur yang dipilih adalah numerik
        if (pd.api.types.is_numeric_dtype(data[x_feature]) and 
            pd.api.types.is_numeric_dtype(data[y_feature])):
            ...
            st.warning("Salah satu fitur yang dipilih bukan numerik, tidak dapat membuat visualisasi")
