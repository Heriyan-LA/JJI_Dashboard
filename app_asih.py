...
from sklearn.tree import plot_tree  # Ditambahkan untuk visualisasi decision tree
...
def load_data():
    ...
    # Asumsi kolom kehadiran dan SKS gagal (untuk visualisasi baru)
    if 'Attendance' not in data.columns:
        # Buat data dummy jika kolom tidak ada
        np.random.seed(42)
        data['Attendance'] = np.random.randint(40, 100, size=len(data))
        
    if 'Failed_credits' not in data.columns:
        # Buat data dummy untuk SKS gagal
        data['Failed_credits'] = np.random.randint(0, 20, size=len(data))
...
# Fungsi untuk visualisasi dasar
...
def plot_additional_visualizations(df):
    """Fungsi untuk visualisasi tambahan: boxplot, stacked bar, dll."""
    
    # 1. Boxplot IPK vs Status Dropout
    st.subheader("Distribusi IPK: Dropout vs Non-Dropout")
    fig1, ax1 = plt.subplots(figsize=(10, 6))
    
    # Pastikan kolom yang diperlukan ada
    if 'Admission_grade' in df.columns and 'Status Group' in df.columns:
        sns.boxplot(
            x='Status Group', 
            y='Admission_grade', 
            data=df, 
        ...
        ax1.set_title("Distribusi IPK Masuk Berdasarkan Status Dropout")
        ax1.set_xlabel("Status")
        ax1.set_ylabel("IPK Masuk")
        st.pyplot(fig1)
        
        # Hitung statistik perbandingan
        non_dropout_mean = df[df['Status Group'] == 'Non-Dropout']['Admission_grade'].mean()
        dropout_mean = df[df['Status Group'] == 'Dropout']['Admission_grade'].mean()
        diff_percent = ((non_dropout_mean - dropout_mean) / non_dropout_mean) * 100
        
        st.info(f"""
        - Rata-rata IPK Non-Dropout: **{non_dropout_mean:.2f}**
        - Rata-rata IPK Dropout: **{dropout_mean:.2f}**
        - Perbedaan: **{diff_percent:.1f}%** (IPK dropout lebih rendah)
        """)
    else:
        st.warning("Kolom 'Admission_grade' atau 'Status Group' tidak ditemukan.")
    
    # 2. Stacked Bar Kehadiran vs DO Rate
    st.subheader("Tingkat Dropout Berdasarkan Kehadiran")
    
    if 'Attendance' in df.columns and 'is_dropout' in df.columns:
        # Buat kategori kehadiran
        df['Attendance_Cat'] = pd.cut(
            df['Attendance'],
            bins=[0, 50, 75, 100],
            labels=['Rendah (<50%)', 'Sedang (50-75%)', 'Tinggi (>75%)']
        )
        
        # Hitung persentase dropout
        attendance_dropout = df.groupby('Attendance_Cat')['is_dropout'].mean().reset_index()
        
        fig2, ax2 = plt.subplots(figsize=(10, 6))
        sns.barplot(
            x='Attendance_Cat', 
            y='is_dropout', 
            data=attendance_dropout,
            ax=ax2,
            palette='viridis',
            order=['Rendah (<50%)', 'Sedang (50-75%)', 'Tinggi (>75%)']
        )
        ax2.set_title("Tingkat Dropout Berdasarkan Persentase Kehadiran")
        ax2.set_xlabel("Tingkat Kehadiran")
        ax2.set_ylabel("Persentase Dropout")
        ax2.set_ylim(0, 1)
        ...
        # Hitung rasio dropout
        low_attendance_rate = attendance_dropout[attendance_dropout['Attendance_Cat'] == 'Rendah (<50%)']['is_dropout'].values[0]
        high_attendance_rate = attendance_dropout[attendance_dropout['Attendance_Cat'] == 'Tinggi (>75%)']['is_dropout'].values[0]
        ratio = low_attendance_rate / high_attendance_rate if high_attendance_rate > 0 else 0
        
        st.info(f"""
        - Dropout rate pada kehadiran rendah: **{low_attendance_rate:.0%}**
        - Dropout rate pada kehadiran tinggi: **{high_attendance_rate:.0%}**
        - Mahasiswa dengan kehadiran rendah **{ratio:.1f}x** lebih mungkin dropout
        """)
    else:
        st.warning("Kolom 'Attendance' atau 'is_dropout' tidak ditemukan. Tidak dapat membuat visualisasi kehadiran.")
    
    # 3. Visualisasi SKS Gagal vs DO Rate
    st.subheader("Tingkat Dropout Berdasarkan Jumlah SKS Gagal")
    
    if 'Failed_credits' in df.columns and 'is_dropout' in df.columns:
        # Buat kategori SKS gagal
        df['Failed_Credits_Cat'] = pd.cut(
            df['Failed_credits'],
            bins=[-1, 0, 5, 10, 100],
            labels=['0 SKS', '1-5 SKS', '6-10 SKS', '>10 SKS']
        )
        
        # Hitung persentase dropout
        failed_credits_dropout = df.groupby('Failed_Credits_Cat')['is_dropout'].mean().reset_index()
        
        fig3, ax3 = plt.subplots(figsize=(10, 6))
        sns.barplot(
            x='Failed_Credits_Cat', 
            y='is_dropout', 
            data=failed_credits_dropout,
            ax=ax3,
            palette='rocket',
            order=['0 SKS', '1-5 SKS', '6-10 SKS', '>10 SKS']
        )
        ax3.set_title("Tingkat Dropout Berdasarkan Jumlah SKS Gagal")
        ax3.set_xlabel("Jumlah SKS Gagal")
        ...
        st.warning("Kolom 'Failed_credits' tidak ditemukan. Tidak dapat membuat visualisasi SKS gagal.")
...
def main():
    ...
    if 'Status' in data.columns:
        ...
        data['Status Group'] = data['Status'].apply(lambda x: 'Dropout' if x == 'Dropout' else 'Non-Dropout')
    ...
    with tab3:
        ...
        st.subheader("Fitur Terpenting dalam Model")
        importance = pd.Series(model.feature_importances_, index=X.columns)
        top_features = importance.sort_values(ascending=False).head(10)
        
        fig_imp, ax_imp = plt.subplots(figsize=(10, 6))
        top_features.plot(kind='barh', ax=ax_imp, color='skyblue')
        ax_imp.set_title("10 Fitur dengan Pengaruh Terbesar")
        st.pyplot(fig_imp)
        
        # Visualisasi Decision Tree
        st.subheader("Struktur Pohon Keputusan (Contoh)")
        
        # Pilih satu pohon dari random forest
        ...
        plot_tree(estimator, 
                  ...
                  impurity=False, 
                  rounded=True,
                  max_depth=3,  # batasi kedalaman agar tidak terlalu besar
                  proportion=True,
                  fontsize=10)
        st.pyplot(plt.gcf())
        st.caption("Contoh satu pohon keputusan dari Random Forest (dibatasi 3 level kedalaman)")
    ...
    with tab5:
        ...
        plot_additional_visualizations(data)
        
        # Kesimpulan analisis
        st.subheader("Kesimpulan Analisis")
        st.markdown("""
        **Dari hasil analisis multivariate ditemukan bahwa:**
        - 📉 **IPK rendah**: Mahasiswa dropout memiliki IPK 15-20% lebih rendah dibandingkan non-dropout
        - ❌ **Ketidakhadiran tinggi**: Dropout rate 3x lebih tinggi pada mahasiswa dengan kehadiran <50%
        ...
        **Faktor pendukung:**
        - 💰 **Faktor finansial**: Mahasiswa dengan tunggakan pembayaran 2.5x lebih mungkin dropout
        - 👪 **Dukungan keluarga**: Korelasi negatif (-0.32) dengan dropout
