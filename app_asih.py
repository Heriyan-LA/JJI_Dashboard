...
def plot_analysis(df):
    ...
    st.subheader("Diagram Pohon Keputusan (Contoh)")
    try:
        ...
        st.warning(f"Tidak dapat menampilkan diagram pohon: {e}")
    
    # 4. Boxplot IPK vs Status
    st.subheader("Distribusi Nilai Masuk Berdasarkan Status")
    if 'Admission_grade' in df.columns and 'Status' in df.columns:
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.boxplot(x='Status', y='Admission_grade', data=df, palette='Set2', ax=ax)
        ax.set_title('Distribusi Nilai Masuk Mahasiswa')
        ax.set_xlabel('Status Mahasiswa')
        ax.set_ylabel('Nilai Masuk (0-200)')
        st.pyplot(fig)
        
        # Statistik
        stats = df.groupby('Status')['Admission_grade'].describe()
        st.write("Statistik Deskriptif Nilai Masuk:")
        st.dataframe(stats)
    else:
        st.warning("Kolom 'Admission_grade' atau 'Status' tidak tersedia untuk boxplot")
    
    # 5. Stacked Bar Kehadiran vs DO Rate
...
with tab1:
    ...
    with st.form("prediction_form"):
        ...
        with col2:
            ...
            grade1 = st.slider("Rata-rata nilai semester 1", 0.0, 20.0, step=0.1, value=12.0)
            grade2 = st.slider("Rata-rata nilai semester 2", 0.0, 20.0, step=0.1, value=13.0)
